...
\node[font=\huge,color=black!25,align=center] at (2, 3) {
    program \\ on\\ disk
};
\end{pgfonlayer}
...
\end{scope}
\begin{visibleenv}<2->
    \fill[color=blue!30] (header tl) rectangle (header br);
...
\begin{visibleenv}<3->
    ...
    \fill[color=red!20] (load off) rectangle ([xshift=4cm]load end);
    \fill[color=red!20] (vaddr off) rectangle ([xshift=-4cm]vaddr end copy);
