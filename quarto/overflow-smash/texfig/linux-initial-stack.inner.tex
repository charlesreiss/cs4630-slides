...
\node[draw,auxv,right=1cm of theStack-7-1] (auxLabel) {
     auxillary vector (see \texttt{man getauxval})
};
\draw[thick] (theStack-7-1.north east) -- (auxLabel.north west);
\draw[thick] (theStack-8-1.south east) -- (auxLabel.south west);

\node[draw,envVarP,right=1cm of theStack-10-1] (envVarPLabel) {
    array of pointers to env. vars.
};
\draw[thick] (theStack-9-1.north east) -- (envVarPLabel.north west);
\draw[thick] (theStack-11-1.south east) -- (envVarPLabel.south west);
\node[draw,argP,right=1cm of theStack-13-1] (argPLabel) {
    array of pointers to args (argv)
};
\draw[thick] (theStack-12-1.north east) -- (argPLabel.north west);
\draw[thick] (theStack-15-1.south east) -- (argPLabel.south west);
\end{tikzpicture}
